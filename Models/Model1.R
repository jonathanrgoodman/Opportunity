...
SR.PD <- function(N=100, generations=1000, mu=.01, delta=.001){
  ...
  for (generations in 2:generations) {
    ...
    #determine payoffs from PD, conditional cooperators do not cooperate with defectors
    
    population$PTR[which(population$payoff=="c c")] <- population$PTR[which(population$payoff=="c c")]+cc
    population$PTR[which(population$payoff=="c d")] <- population$PTR[which(population$payoff=="c d")]+dd
    population$PTR[which(population$payoff=="d c")] <- population$PTR[which(population$payoff=="d c")]+dd
    population$PTR[which(population$payoff=="d d")] <- population$PTR[which(population$payoff=="d d")]+dd
    
    #cheated cooperators punish defectors
    
    population$PTR[which(population$payoff=="c d")] <- population$PTR[which(population$payoff=="c d")]+punish
    population$PTR[which(population$payoff=="d c")] <- population$PTR[which(population$payoff=="d c")]+cost
    
    #determine % of cooperation in population, slot into cooperation output
    
    coop <- length(which(population$payoff=="c c"))
