...
mimic.PD.3 <- function(N=100, generations=1000, mu=.01, delta=.001){
  ...
  #ignore mimicry in this model; sensitivity is a probability only
  ...
  population$sensitivity <- round(rnorm(N,mean=.65,sd=.1), digits=2)
  
  #ensure no individual-level parameter values are below 0 or above 1; ensure sensitivity is at least .01 and no higher than .99
  ...
  population$sensitivity[population$sensitivity>1] <- .99
  population$sensitivity[population$sensitivity<=0] <- .01
  ...
  for (generations in 1:generations) {
    ...
    #probability function for cheater detection; focal individual punishes mimic and mimic is coerced into cooperating
    
    #create vector for storing conditional defector IDs
    
    conditional.defectors <- NA
    
    for (i in 1:nrow(group1)){
    
    if(sample(1:2, 1, prob=c(group1$sensitivity[i],1-group1$sensitivity[i]))==1 & group1$strategy[i]=="c" & group2$type[i]=="m" & group2$strategy[i]=="d"){
      
    group1$PTR[i] <- group1$PTR[i] + punish
    group2$PTR[i] <- group2$PTR[i] + cost
    
    group2$strategy[i] <- "c"
  
    ifelse(any(is.na(conditional.defectors)),conditional.defectors <- group2$individual[i],conditional.defectors <- c(conditional.defectors, group2$individual[i]))
    ...
    for (i in 1:nrow(group2)){
      ...
      if(sample(1:2, 1, prob=c(group2$sensitivity[i],1-group2$sensitivity[i]))==1 & group2$strategy[i]=="c" & group1$type[i]=="m" & group1$strategy[i]=="d"){
        
        group2$PTR[i] <- group2$PTR[i] + punish
        group1$PTR[i] <- group1$PTR[i] + cost
        
        group1$strategy[i] <- "c"
        
        ifelse(any(is.na(conditional.defectors)),conditional.defectors <- group1$individual[i],conditional.defectors <- c(conditional.defectors, group1$individual[i]))
        
    }
    }
    
    
    #order individual IDs of conditional defectors
    ...
    #adjust PTR and sensitivity by f in next generation
